...
Note: the video will be available in console as 'video'`,"");if(p===null)return;let E=document.createElement("video");E.src=p,E.classList.add("secret_video"),a.parentElement.appendChild(E),E.play(),window.video=E,this.seq&&(E.currentTime=parseFloat(window.prompt("Video offset to sync to midi, in seconds.","0")),E.play(),this.seq.currentTime=0),document.addEventListener("keydown",B=>{B.key===" "&&(E.paused?E.play():E.pause())});break;case Ee.sustainPedal:this.renderer.showHoldPedal=!0,this.renderer.render(!1),this.keyboard.setHoldPedal(!0)}}),document.addEventListener("keyup",g=>{if(!g.ctrlKey)switch(g.key.toLowerCase()){case Ee.sustainPedal:this.renderer.showHoldPedal=!1,this.renderer.render(!1),this.keyboard.setHoldPedal(!1);break;default:break}}),this.renderer.render(!1,!0)}doDLSCheck(){if(window.isLocalEdition!==!0){let e=this.soundFont.slice(8,12);ce(new F(e),4).toLowerCase()==="dls "&&re(this.localeManager.getLocaleString("locale.convertDls.title"),[{type:"text",textContent:this.localeManager.getLocaleString("locale.convertDls.message")},{type:"button",textContent:this.localeManager.getLocaleString("locale.yes"),onClick:i=>{ne(i.id),this.downloadDesfont()}},{type:"button",textContent:this.localeManager.getLocaleString("locale.no"),onClick:i=>{ne(i.id)}}],99999999)}}async reloadSf(e){await this.synth.soundfontManager.reloadManager(e),this.soundFont=e,setTimeout(()=>{this.doDLSCheck()},3e3)}play(e){if(this.synth){if(this.seq){this.seq.loadNewSongList(e),this.seq.play(!0);return}this.seq=new Jo(e,this.synth),this.seq.onError=t=>{document.getElementById("title").textContent=t},this.seqUI.connectSequencer(this.seq),this.playerUI.connectSequencer(this.seq),this.renderer.connectSequencer(this.seq),this.settingsUI.addSequencer(this.seq),this.seq.play(!0)}}async downloadDLSRMI(){let e=await this.seq.getMIDI(),t=mt(this.soundFont),i=Vo(t.writeDLS(),e,t),n=new Blob([i.buffer],{type:"audio/rmid"});this.saveBlob(n,`${e.midiName}.rmi`)}downloadDesfont(){let e=mt(this.soundFont),t=e.write(),i=new Blob([t.buffer],{type:"audio/soundfont"});this.saveBlob(i,`${e.soundFontInfo.INAM}.sf2`)}};it.prototype.exportSong=Or;it.prototype._exportAudioData=er;it.prototype._doExportAudioData=$A;it.prototype.exportMidi=tr;it.prototype._exportSoundfont=Pr;it.prototype._exportDLS=Jr;it.prototype._exportRMIDI=Yr;var Ya=44100,Va="GeneralUserGS.sf3",Ei=document.getElementById("title"),vt=document.getElementById("midi_file_input"),Ja=document.getElementById("sf_file_input"),Kr=document.getElementById("demo_song"),tn=document.getElementById("export_button"),Ot=document.getElementsByClassName("loading")[0],Pe=document.getElementById("loading_message"),Ka=await(await fetch("package.json")).json();window.SPESSASYNTH_VERSION=Ka.version;var za="spessasynth-db",Vt="soundFontStore";function sn(o){let e=indexedDB.open(za,1);e.onsuccess=()=>{let t=e.result;o(t)},e.onupgradeneeded=t=>{t.target.result.createObjectStore(Vt,{keyPath:"id"})}}async function Wa(){return await new Promise(o=>{sn(e=>{let n=e.transaction([Vt],"readonly").objectStore(Vt).get("buffer");n.onerror=s=>{console.error("Database error"),console.error(s),o(void 0)},n.onsuccess=async()=>{let s=n.result;if(!s){o(void 0);return}o(s.data)}})})}function Yt(o,e=!0){let t=Ot.getElementsByClassName("loading_icon")[0];t.innerHTML=o,t.style.animation=e?"none":""}async function zr(o){sn(e=>{let i=e.transaction([Vt],"readwrite").objectStore(Vt);try{let n=i.put({id:"buffer",data:o});n.onsuccess=()=>{M("SoundFont stored successfully")},n.onerror=s=>{console.error("Error saving soundfont",s)}}catch(n){Z("Failed saving soundfont:",n)}})}async function Za(o){let e=new Ai(o);try{let n=window.AudioContext||window.webkitAudioContext;window.audioContextMain=new n({sampleRate:Ya})}catch(n){throw Yt(po(256)),Pe.textContent=e.getLocaleString("locale.synthInit.noWebAudio"),n}Pe.textContent=e.getLocaleString("locale.synthInit.loadingSoundfont");let t=await Wa(),i=!0;if(t===void 0){Z("Failed to load from db, fetching online instead"),i=!1;let n=document.getElementById("progress_bar"),s=e.getLocaleString("locale.synthInit.loadingBundledSoundfont");Pe.textContent=s,t=await ja(`soundfonts/${Va}`,A=>{Pe.textContent=`${s} ${A}%`}),n.style.width="0"}else M("Loaded the soundfont from the database succesfully");window.soundFontParser=t,i||(Pe.textContent=e.getLocaleString("locale.synthInit.savingSoundfont"),await zr(t)),window.audioContextMain.state!=="running"&&document.addEventListener("mousedown",()=>{window.audioContextMain.state!=="running"&&window.audioContextMain.resume().then()}),Pe.textContent=e.getLocaleString("locale.synthInit.startingSynthesizer"),window.manager=new it(audioContextMain,soundFontParser,e),window.manager.sfError=n=>{Yt(po(256)),i?(Z("Invalid soundfont in the database. Resetting."),sn(s=>{let r=s.transaction([Vt],"readwrite").objectStore(Vt).delete("buffer");r.onsuccess=()=>{location.reload()}})):Ei.innerHTML=`Error parsing soundfont: <pre style='font-family: monospace; font-weight: bold'>${n}</pre>`,Pe.innerHTML=`Error parsing soundfont: <pre style='font-family: monospace; font-weight: bold'>${n}</pre>`},await manager.ready,vt.files[0]?await on(vt.files):(vt.onclick=void 0,vt.onchange=()=>{vt.files[0]&&on(vt.files).then()}),Yt(qi(256)),Pe.textContent=e.getLocaleString("locale.synthInit.done")}async function ja(o,e){let t=await fetch(o);if(!t.ok)throw Ei.innerText="Error downloading soundfont!",t;let i=t.headers.get("content-length"),n=await(await t.body).getReader(),s=!1,A=new Uint8Array(parseInt(i)),l=0;do{let r=await n.read();r.value&&(A.set(r.value,l),l+=r.value.length),s=r.done;let a=Math.round(l/i*100);e(a)}while(!s);return A.buffer}async function on(o){Kr.style.display="none";let e;o[0].name.length>20?e=o[0].name.substring(0,21)+"...":e=o[0].name,o.length>1&&(e+=` and ${o.length-1} others`),document.getElementById("file_upload").innerText=e,document.getElementById("file_upload").title=o[0].name;let t=[];for(let i of o)t.push({binary:await i.arrayBuffer(),altName:i.name});manager.synth.setLogLevel(!1,!1,!1,!1),manager.seq?manager.seq.loadNewSongList(t):manager.play(t),tn.style.display="flex",tn.onclick=window.manager.exportSong.bind(window.manager)}function Xa(o){localStorage.setItem("spessasynth-settings",JSON.stringify(o)),M("saved as",o)}window.saveSettings=Xa;var yo=JSON.parse(localStorage.getItem("spessasynth-settings"));yo!==null&&(window.savedSettings=new Promise(o=>{o(yo)}));var nn;yo&&yo.interface&&yo.interface.language?nn=(await savedSettings).interface.language||navigator.language.split("-")[0].toLowerCase():nn=navigator.language.split("-")[0].toLowerCase();vt.value="";vt.focus();tn.style.display="none";document.getElementById("sf_upload").style.display="none";document.getElementById("file_upload").style.display="none";async function $a(o){Ei.textContent=window.manager.localeManager.getLocaleString("locale.synthInit.genericLoading");let e=await fetch("https://spessasus.github.io/spessasynth-demo-songs/demo_songs/"+o);e.name=o,await on([e])}Za(nn).then(()=>{document.getElementById("sf_upload").style.display="flex",document.getElementById("file_upload").style.display="flex",Ot.classList.add("done"),document.documentElement.classList.add("no_scroll"),document.body.classList.add("no_scroll"),setTimeout(()=>{Ot.style.display="none",document.body.classList.remove("no_scroll"),document.documentElement.classList.remove("no_scroll"),qe&&window.chrome&&re(window.manager.localeManager.getLocaleString("locale.warnings.warning"),[{type:"text",textContent:window.manager.localeManager.getLocaleString("locale.warnings.chromeMobile")}],7)},1e3),Ja.onchange=o=>{if(!o.target.files[0])return;let e=o.target.files[0];window.manager.seq&&window.manager.seq.pause(),document.getElementById("sf_upload").firstElementChild.innerText=e.name,Ot.style.display="",setTimeout(async()=>{Ot.classList.remove("done"),Yt(ls(256),!1),Pe.textContent=window.manager.localeManager.getLocaleString("locale.synthInit.loadingSoundfont");let t=performance.now()/1e3,i;try{i=await e.arrayBuffer(),window.soundFontParser=i}catch(s){throw Pe.textContent=window.manager.localeManager.getLocaleString("locale.warnings.outOfMemory"),Yt(po(256)),re(manager.localeManager.getLocaleString("locale.warnings.warning"),[{type:"text",textContent:window.manager.localeManager.getLocaleString("locale.warnings.outOfMemory")}]),s}window.manager.sfError=s=>{Pe.innerHTML=`Error parsing soundfont: <pre style='font-family: monospace; font-weight: bold'>${s}</pre>`,Yt(po(256)),console.error(s)},Pe.textContent=window.manager.localeManager.getLocaleString("locale.synthInit.startingSynthesizer"),await window.manager.reloadSf(i),window.manager.seq&&(window.manager.seq.currentTime-=.1),Pe.textContent=window.manager.localeManager.getLocaleString("locale.synthInit.savingSoundfont"),await zr(i);let n=performance.now()/1e3-t;await new Promise(s=>setTimeout(s,1e3-n)),Yt(qi(256)),Pe.textContent=window.manager.localeManager.getLocaleString("locale.synthInit.done"),Ot.classList.add("done"),document.documentElement.classList.add("no_scroll"),document.body.classList.add("no_scroll"),setTimeout(()=>{Ot.style.display="none",document.body.classList.remove("no_scroll"),document.documentElement.classList.remove("no_scroll")},1e3)},75)},Kr.onclick=async()=>{let o=[{type:"button",textContent:"Bundled SoundFont Credits",onClick:()=>{window.open("https://schristiancollins.com/generaluser.php")}}];Ei.textContent=window.manager.localeManager.getLocaleString("locale.synthInit.genericLoading");let e=await(await fetch("https://spessasus.github.io/spessasynth-demo-songs/demo_song_list.json")).text(),t=JSON.parse(e);for(let i of t)o.push({type:"button",textContent:i.name,onClick:n=>{ne(n.id),re(window.manager.localeManager.getLocaleString("locale.credits"),[{type:"text",textContent:i.credits.replace(`
`,`\r
\r
