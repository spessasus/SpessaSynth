...
Note: the video will be available in console as 'video'`,"");if(C===null)return;let p=document.createElement("video");p.src=C,p.classList.add("secret_video"),d.parentElement.appendChild(p),p.play(),window.video=p,this.seq&&(p.currentTime=parseFloat(window.prompt("Video offset to sync to midi, in seconds.","0")),p.play(),this.seq.currentTime=0),document.addEventListener("keydown",B=>{B.key===" "&&(p.paused?p.play():p.pause())});break;case fe.sustainPedal:this.renderer.showHoldPedal=!0,this.renderer.render(!1),this.keyboard.setHoldPedal(!0)}}),document.addEventListener("keyup",g=>{if(!g.ctrlKey)switch(g.key.toLowerCase()){case fe.sustainPedal:this.renderer.showHoldPedal=!1,this.renderer.render(!1),this.keyboard.setHoldPedal(!1);break;default:break}}),this.renderer.render(!1,!0)}doDLSCheck(){if(window.isLocalEdition!==!0){let e=this.soundFont.slice(8,12);ae(new L(e),4).toLowerCase()==="dls "&&re(this.localeManager.getLocaleString("locale.convertDls.title"),[{type:"text",textContent:this.localeManager.getLocaleString("locale.convertDls.message")},{type:"button",textContent:this.localeManager.getLocaleString("locale.yes"),onClick:i=>{ne(i.id),this.downloadDesfont()}},{type:"button",textContent:this.localeManager.getLocaleString("locale.no"),onClick:i=>{ne(i.id)}}],99999999)}}async reloadSf(e){await this.synth.soundfontManager.reloadManager(e),this.soundFont=e,setTimeout(()=>{this.doDLSCheck()},3e3)}play(e){if(this.synth){if(this.seq){this.seq.loadNewSongList(e),this.seq.play(!0);return}this.seq=new Wo(e,this.synth),this.seq.onError=t=>{document.getElementById("title").textContent=t},this.seqUI.connectSequencer(this.seq),this.playerUI.connectSequencer(this.seq),this.renderer.connectSequencer(this.seq),this.settingsUI.addSequencer(this.seq),this.seq.play(!0)}}async downloadDLSRMI(){let e=await this.seq.getMIDI(),t=mt(this.soundFont),i=Ko(t.writeDLS(),e,t),n=new Blob([i.buffer],{type:"audio/rmid"});this.saveBlob(n,`${e.midiName}.rmi`)}downloadDesfont(){let e=mt(this.soundFont),t=e.write(),i=new Blob([t.buffer],{type:"audio/soundfont"});this.saveBlob(i,`${e.soundFontInfo.INAM}.sf2`)}};it.prototype.exportSong=Kr;it.prototype._exportAudioData=sr;it.prototype._doExportAudioData=nr;it.prototype.exportMidi=Ar;it.prototype._exportSoundfont=Vr;it.prototype._exportDLS=jr;it.prototype._exportRMIDI=Wr;var Za=44100,ja="GeneralUserGS.sf3",Qi=document.getElementById("title"),Mt=document.getElementById("midi_file_input"),Xa=document.getElementById("sf_file_input"),Xr=document.getElementById("demo_song"),nn=document.getElementById("export_button"),Yt=document.getElementsByClassName("loading")[0],Oe=document.getElementById("loading_message"),$a=await(await fetch("package.json")).json();window.SPESSASYNTH_VERSION=$a.version;var el="spessasynth-db",Jt="soundFontStore";function rn(o){let e=indexedDB.open(el,1);e.onsuccess=()=>{let t=e.result;o(t)},e.onupgradeneeded=t=>{t.target.result.createObjectStore(Jt,{keyPath:"id"})}}async function tl(){return await new Promise(o=>{rn(e=>{let n=e.transaction([Jt],"readonly").objectStore(Jt).get("buffer");n.onerror=s=>{console.error("Database error"),console.error(s),o(void 0)},n.onsuccess=async()=>{let s=n.result;if(!s){o(void 0);return}o(s.data)}})})}function zt(o,e=!0){let t=Yt.getElementsByClassName("loading_icon")[0];t.innerHTML=o,t.style.animation=e?"none":""}async function $r(o){rn(e=>{let i=e.transaction([Jt],"readwrite").objectStore(Jt);try{let n=i.put({id:"buffer",data:o});n.onsuccess=()=>{M("SoundFont stored successfully")},n.onerror=s=>{console.error("Error saving soundfont",s)}}catch(n){Z("Failed saving soundfont:",n)}})}async function ol(o){let e=new li(o);try{let n=window.AudioContext||window.webkitAudioContext;window.audioContextMain=new n({sampleRate:Za})}catch(n){throw zt(Bo(256)),Oe.textContent=e.getLocaleString("locale.synthInit.noWebAudio"),n}Oe.textContent=e.getLocaleString("locale.synthInit.loadingSoundfont");let t=await tl(),i=!0;if(t===void 0){Z("Failed to load from db, fetching online instead"),i=!1;let n=document.getElementById("progress_bar"),s=e.getLocaleString("locale.synthInit.loadingBundledSoundfont");Oe.textContent=s,t=await il(`soundfonts/${ja}`,r=>{Oe.textContent=`${s} ${r}%`}),n.style.width="0"}else M("Loaded the soundfont from the database succesfully");window.soundFontParser=t,i||(Oe.textContent=e.getLocaleString("locale.synthInit.savingSoundfont"),await $r(t)),window.audioContextMain.state!=="running"&&document.addEventListener("mousedown",()=>{window.audioContextMain.state!=="running"&&window.audioContextMain.resume().then()}),Oe.textContent=e.getLocaleString("locale.synthInit.startingSynthesizer"),window.manager=new it(audioContextMain,soundFontParser,e),window.manager.sfError=n=>{zt(Bo(256)),i?(Z("Invalid soundfont in the database. Resetting."),rn(s=>{let A=s.transaction([Jt],"readwrite").objectStore(Jt).delete("buffer");A.onsuccess=()=>{location.reload()}})):Qi.innerHTML=`Error parsing soundfont: <pre style='font-family: monospace; font-weight: bold'>${n}</pre>`,Oe.innerHTML=`Error parsing soundfont: <pre style='font-family: monospace; font-weight: bold'>${n}</pre>`},await manager.ready,Mt.files[0]?await sn(Mt.files):(Mt.onclick=void 0,Mt.onchange=()=>{Mt.files[0]&&sn(Mt.files).then()}),zt(Pi(256)),Oe.textContent=e.getLocaleString("locale.synthInit.done")}async function il(o,e){let t=await fetch(o);if(!t.ok)throw Qi.innerText="Error downloading soundfont!",t;let i=t.headers.get("content-length"),n=await(await t.body).getReader(),s=!1,r=new Uint8Array(parseInt(i)),a=0;do{let A=await n.read();A.value&&(r.set(A.value,a),a+=A.value.length),s=A.done;let d=Math.round(a/i*100);e(d)}while(!s);return r.buffer}async function sn(o){Xr.style.display="none";let e;o[0].name.length>20?e=o[0].name.substring(0,21)+"...":e=o[0].name,o.length>1&&(e+=` and ${o.length-1} others`),document.getElementById("file_upload").innerText=e,document.getElementById("file_upload").title=o[0].name;let t=[];for(let i of o)t.push({binary:await i.arrayBuffer(),altName:i.name});manager.synth.setLogLevel(!1,!1,!1,!1),manager.seq?manager.seq.loadNewSongList(t):manager.play(t),nn.style.display="flex",nn.onclick=window.manager.exportSong.bind(window.manager)}function nl(o){localStorage.setItem("spessasynth-settings",JSON.stringify(o)),M("saved as",o)}window.saveSettings=nl;var ko=JSON.parse(localStorage.getItem("spessasynth-settings"));ko!==null&&(window.savedSettings=new Promise(o=>{o(ko)}));var An;ko&&ko.interface&&ko.interface.language?An=(await savedSettings).interface.language||navigator.language.split("-")[0].toLowerCase():An=navigator.language.split("-")[0].toLowerCase();Mt.value="";Mt.focus();nn.style.display="none";document.getElementById("sf_upload").style.display="none";document.getElementById("file_upload").style.display="none";async function sl(o){Qi.textContent=window.manager.localeManager.getLocaleString("locale.synthInit.genericLoading");let e=await fetch("https://spessasus.github.io/spessasynth-demo-songs/demo_songs/"+o);e.name=o,await sn([e])}ol(An).then(()=>{document.getElementById("sf_upload").style.display="flex",document.getElementById("file_upload").style.display="flex",Yt.classList.add("done"),document.documentElement.classList.add("no_scroll"),document.body.classList.add("no_scroll"),setTimeout(()=>{Yt.style.display="none",document.body.classList.remove("no_scroll"),document.documentElement.classList.remove("no_scroll"),_e&&window.chrome&&re(window.manager.localeManager.getLocaleString("locale.warnings.warning"),[{type:"text",textContent:window.manager.localeManager.getLocaleString("locale.warnings.chromeMobile")}],7)},1e3),Xa.onchange=o=>{if(!o.target.files[0])return;let e=o.target.files[0];window.manager.seq&&window.manager.seq.pause(),document.getElementById("sf_upload").firstElementChild.innerText=e.name,Yt.style.display="",setTimeout(async()=>{Yt.classList.remove("done"),zt(cs(256),!1),Oe.textContent=window.manager.localeManager.getLocaleString("locale.synthInit.loadingSoundfont");let t=performance.now()/1e3,i;try{i=await e.arrayBuffer(),window.soundFontParser=i}catch(s){throw Oe.textContent=window.manager.localeManager.getLocaleString("locale.warnings.outOfMemory"),zt(Bo(256)),re(manager.localeManager.getLocaleString("locale.warnings.warning"),[{type:"text",textContent:window.manager.localeManager.getLocaleString("locale.warnings.outOfMemory")}]),s}window.manager.sfError=s=>{Oe.innerHTML=`Error parsing soundfont: <pre style='font-family: monospace; font-weight: bold'>${s}</pre>`,zt(Bo(256)),console.error(s)},Oe.textContent=window.manager.localeManager.getLocaleString("locale.synthInit.startingSynthesizer"),await window.manager.reloadSf(i),window.manager.seq&&(window.manager.seq.currentTime-=.1),Oe.textContent=window.manager.localeManager.getLocaleString("locale.synthInit.savingSoundfont"),await $r(i);let n=performance.now()/1e3-t;await new Promise(s=>setTimeout(s,1e3-n)),zt(Pi(256)),Oe.textContent=window.manager.localeManager.getLocaleString("locale.synthInit.done"),Yt.classList.add("done"),document.documentElement.classList.add("no_scroll"),document.body.classList.add("no_scroll"),setTimeout(()=>{Yt.style.display="none",document.body.classList.remove("no_scroll"),document.documentElement.classList.remove("no_scroll")},1e3)},75)},Xr.onclick=async()=>{let o=[{type:"button",textContent:"Bundled SoundFont Credits",onClick:()=>{window.open("https://schristiancollins.com/generaluser.php")}}];Qi.textContent=window.manager.localeManager.getLocaleString("locale.synthInit.genericLoading");let e=await(await fetch("https://spessasus.github.io/spessasynth-demo-songs/demo_song_list.json")).text(),t=JSON.parse(e);for(let i of t)o.push({type:"button",textContent:i.name,onClick:n=>{ne(n.id),re(window.manager.localeManager.getLocaleString("locale.credits"),[{type:"text",textContent:i.credits.replace(`
`,`\r
\r
