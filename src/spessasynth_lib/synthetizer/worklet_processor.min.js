...
Converted from DLS to SF2 with SpessaSynth`;for(let[C,B]of Object.entries(this.soundFontInfo))p(`%c"${C}": %c"${B}"`,g.info,g.recognized);let o=n.find(C=>C.header==="colh");o||(V(),this.parsingError("No colh chunk!")),this.instrumentAmount=F(o.chunkData,4),p(`%cInstruments amount: %c${this.instrumentAmount}`,g.info,g.recognized);let i=MA(n,"wvpl");i||(V(),this.parsingError("No wvpl chunk!")),this.readDLSSamples(i);let a=MA(n,"lins");a||(V(),this.parsingError("No lins chunk!")),this.readDLSInstrumentList(a),this.presets.sort((C,B)=>C.program-B.program+(C.bank-B.bank)),p(`%cParsing finished! %c"${this.soundFontInfo.INAM||"UNNAMED"}"%c has %c${this.presets.length} %cpresets,
